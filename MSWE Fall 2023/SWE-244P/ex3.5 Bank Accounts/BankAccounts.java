...
public class BankAccounts {
	private static final Object tieLock = new Object(); //tieLock in the case hashcodes of acc1 and acc2 are the same
	public static void transferMoney(Account fromAccount,
                                     Account toAccount,
                                     DollarAmount amount)
            throws InsufficientFundsException {
		class Helper { // inducing lock ordering from textbook
...
//	 }
	...
	public Teller(int i, Account f, Account t) {
	    id = i;
	    from = f; //(f.getBalance().get() >= t.getBalance().get())
	    to = t;
	}
	...
	// One teller one way
	// want to prevent dynamic lock-ordering deadlock, solve by inducing a lock ordering inside Teller
